...
function DynamoBackup(options) {
    ...
    this.useAwsCreds = (typeof options.useAwsCreds != 'undefined' ? options.useAwsCreds : true);
    ...
    var config = new AWS.Config(params);
    AWS.config = config;
}
...
DynamoBackup.prototype.backupTable = function (tableName, backupPath, callback) {
    ...
    s3Conf.region = self.awsRegion;
    if (self.useAwsCreds) {
        s3Conf.accessKeyId = self.awsAccessKey;
        s3Conf.secretAccessKey = self.awsSecretKey;
    }
    var Uploader = new AWS.S3(s3Conf);

    params.Bucket = self.bucket;
    params.Key = path.join(backupPath, tableName + '.json');
    params.Body = stream;
